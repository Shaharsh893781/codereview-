...
async function register() {
  const data = await api("/api/auth/register", {
    method: "POST",
    body: JSON.stringify({ email: qs("#email").value, full_name: qs("#name").value, password: qs("#password").value }),
  });
...
function renderResult(result) {
  ...
  qs("#aiTab").innerHTML = result.ai_suggestions.map(item => `<article class="issue"><p>${escapeHtml(item)}</p></article>`).join("");
  renderCharts(result);
}
...
async function loadHistory() {
  ...
  qs("#historyTab").innerHTML = scans.map(scan => `
    <article class="issue">
      ...
      <button class="ghost report-btn" data-id="${scan.id}" data-kind="json">JSON</button>
      <button class="ghost report-btn" data-id="${scan.id}" data-kind="pdf">PDF</button>
    </article>
...
qs("#registerBtn").addEventListener("click", () => register().catch(error => setAuthStatus(error.message)));
qs("#loginBtn").addEventListener("click", () => login().catch(error => setAuthStatus(error.message)));
...
if (token) {
  setAuthStatus("Signed in");
  loadHistory().catch(() => setAuthStatus("Session expired"));
}
